...
## Matriz diagnostica incorporada

O app agora apresenta uma matriz `premissa -> diagnostico -> problema predominante -> sugestao`. Essa matriz materializa a limitacao metodologica do PLN lexical e separa problemas de redacao do usuario, lacunas de lexico/regras e inferencias ja suficientemente rastreaveis.

Premissas como `compensatoriedade`, `monotonicidade`, `ponderabilidade` e `completude_pref` recebem atencao especial, pois tendem a depender mais de expressoes compostas e contexto semantico do que atributos mais diretos como `tipo_problema` e `ambiente_decisao`.
